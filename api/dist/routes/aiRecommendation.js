...
router.post('/recommendation', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        try {
            const { data: cachedData, error: cacheError } = await supabase_1.supabase
                .from(supabase_1.TABLES.AI_RECOMMENDATIONS)
        ...
        const calculateVolatility = (history) => {
            if (history.length < 2)
                return { volatility: 0, trend: 'flat', highestPrice: safeCurrentPrice, daysSinceLastDrop: 0, avgPrice: safeCurrentPrice, predictedDrop: 'Unknown' };
            ...
            const lastDropIndex = sorted.findIndex((h, i) => i > 0 && h.price < sorted[i - 1].price);
            ...
            if (trend === 'falling')
                predictedDrop = 'Price is currently dropping';
            else if (daysSinceLastDrop > 14)
                predictedDrop = 'Due for a drop soon (based on 2-week cycle)';
            else if (isEndMonth)
                predictedDrop = 'Likely to drop start of next month';
            return {
        ...
        const detectConditionClues = (title) => {
            const lowerTitle = title.toLowerCase();
            const clues = [];
            if (lowerTitle.includes('refurbished'))
                clues.push('Refurbished');
            ...
            if (lowerTitle.includes('used'))
                clues.push('Used');
            if (lowerTitle.includes('renewed'))
                clues.push('Amazon Renewed');
            if (lowerTitle.includes('no charger') || lowerTitle.includes('without charger'))
                clues.push('No charger included');
            if (lowerTitle.includes('cracked') || lowerTitle.includes('damaged'))
                clues.push('⚠️ Damage mentioned');
            if (lowerTitle.includes('amazon'))
                clues.push('Sold by Amazon');
            if (lowerTitle.includes('3rd party') || lowerTitle.includes('third party'))
                clues.push('3rd party seller');
            if (lowerTitle.includes('only') && lowerTitle.includes('left'))
                clues.push('⏰ Low stock - Urgency High');
            if (lowerTitle.includes('limited'))
                clues.push('⏰ Limited offer');
            return clues;
        };
        ...
        if (priceAnalysis.volatility > 15)
            risk = 'High';
        else if (priceAnalysis.volatility > 8)
            risk = 'Medium';
