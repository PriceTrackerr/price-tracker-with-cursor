...
console.log('[DB INIT] USE_SUPABASE:', USE_SUPABASE, 'USE_LOCAL_DB:', USE_LOCAL_DB);

// ---------------------
// Local File Storage Setup
// ---------------------
let DATA_FILE: string | null = null;
...
class FileStorage {
  private readData() {
    if (!DATA_FILE) throw new Error('FileStorage not initialized');
    try {
      const data = fs.readFileSync(DATA_FILE, 'utf8');
      return JSON.parse(data);
    } catch (error) {
      console.error('[FileStorage] Error reading data file:', error);
      return {
        products: [],
        users: [],
        alerts: [],
  ...
  private writeData(data: any) {
    ...
    try {
      ...
      console.error('[FileStorage] Error writing data file:', error);
      throw error;
    }
  }
  ...
  async getUserByEmail(email: string) {
    return this.readData().users.find((u: any) => u.email === email);
  }

  async getUserById(id: string) {
    return this.readData().users.find((u: any) => u.id === id);
  }
  ...
  async addProduct(productData: any) {
    ...
    const id = Date.now().toString() + Math.random().toString(36).slice(2, 8);
    const now = new Date().toISOString();
    const product = { id, ...productData, stockStatus: productData.stockStatus || 'unknown', createdAt: now, updatedAt: now };
    data.products.push(product);
    this.writeData(data);
    return id;
  }
  ...
  async getProducts(userId?: string) {
    const data = this.readData();
    return userId ? data.products.filter((p: any) => p.userId === userId) : data.products;
  }

  async getProductById(id: string) {
    return this.readData().products.find((p: any) => p.id === id);
  }
  ...
  async getAlerts(userId?: string) {
    const data = this.readData();
    return userId ? data.alerts.filter((a: any) => a.userId === userId) : data.alerts;
  }

  async updateAlert(id: string, update: any) {
    const data = this.readData();
  ...
  async deleteAlert(id: string) {
    const data = this.readData();
    const initialLength = data.alerts.length;
    data.alerts = data.alerts.filter((a: any) => a.id !== id);
    this.writeData(data);
    return data.alerts.length !== initialLength;
  }
  ...
  async getNotifications(userId?: string) {
    const data = this.readData();
    return userId ? data.notifications.filter((n: any) => n.userId === userId) : data.notifications;
  }

  async updateNotification(id: string, update: any) {
    const data = this.readData();
  ...
  // PRICE HISTORY
  // ---------------------
  async addPriceHistory(historyData: any) {
    const data = this.readData();
    const id = Date.now().toString() + Math.random().toString(36).slice(2, 8);
    const now = new Date().toISOString();
  ...
  async getPriceHistory(productId: string) {
    const data = this.readData();
    return data.priceHistory.filter((h: any) => h.productId === productId);
  }

  // ---------------------
  // Payments / Affiliate / Payouts
  // ---------------------
  ...
  async getUserPayments(userId: string) {
    const data = this.readData();
    return data.payments.filter((p: any) => p.userId === userId);
  }

  async addAffiliateTransaction(transactionData: any) {
    const data = this.readData();
    data.affiliateTransactions.push(transactionData);
    this.writeData(data);
    return transactionData.id;
  ...
  async addPayoutRequest(payoutData: any) {
    const data = this.readData();
    data.payoutRequests.push(payoutData);
    this.writeData(data);
    return payoutData.id;
...
if (USE_LOCAL_DB) {
  console.log('[DB INIT] ✅ Using local FileStorage');
  db = new FileStorage();
} else if (USE_SUPABASE && isSupabaseConfigured()) {
  console.log('[DB INIT] ✅ Using Supabase (Production)');
  db = supabaseStorage;
} else {
  console.warn('[DB INIT] ⚠️ Supabase not configured. Falling back to local FileStorage');
  db = new FileStorage();
}
