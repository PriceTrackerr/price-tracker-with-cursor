...
function addAffiliateTag(url: string, platform: 'amazon' | 'aliexpress' | 'ebay' | 'walmart' | 'shein' | 'bestbuy' | 'target'): string {
  if (platform === 'amazon') {
    ...
    const sep = hasQuery ? '&' : '?';
    // Basic affiliate parameters; adjust when you have a specific program key
...
// Email-Safe CSS Styles (Works in all email clients)
...
class EmailService {
  ...
  private getPlatformClass(platform: string): string {
  ...
  async sendConsolidatedPriceDropAlert(
    userEmail: string,
    priceDrops: Array<{
      productTitle: string;
      currentPrice: number;
