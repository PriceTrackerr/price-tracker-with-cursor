...
export function ReconnectingIndicator({ reconnecting }) {
    if (!reconnecting)
        return null;
    return (_jsxs("div", { className: "fixed top-4 right-4 z-50 bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2", children: [_jsx(Wifi, { className: "w-4 h-4 animate-pulse" }), _jsx("span", { className: "text-sm font-medium", children: "Reconnecting..." })] }));
}
export function AuthProvider({ children }) {
    ...
    const fetchUser = useCallback(async (t, retryCount = 0) => {
    ...
    const syncTokenToExtension = useCallback(async (tokenToSync) => {
        try {
            if (typeof chrome !== 'undefined' && chrome.storage) {
    ...
    const bootstrapSession = useCallback(async (accessToken, refreshTokenValue) => {
    ...
    return (_jsx(AuthContext.Provider, { value: { user, token, loading, reconnecting, login, signup, logout, getAuthHeaders, bootstrapSession }, children: children }));
}
